...
u_int32 resolveLabel (char *label)
{
  ...
  if (entry == NULL)
  {
    fprintf (stderr, "Label (%s) is not declared\n", label);
    exit (ERROR);
  }
...
void genIType (u_int32 kop, u_int32 rt, u_int32 rs, u_int32 imm)
{
  ...
  if (pass != SYMBOL_PASS)
  {
    ...
    {
      case kBEQ:
        packed_opcode = formI (0x04, rs, rt, offset);
        break;
      case kBNE:
        packed_opcode = formI (0x05, rs, rt, offset);
        break;
      case kADDI:
                        ...
                        char *debug_str)
{
  ...
  else if (output_type == OUTPUT_INTELHEX64)
  {
    /* vars to compute checksum and addr */
    int i, j, checksum = 0;
    /* hex format address is 16 bits
...
int32 symTableEnter (SymEntry *entry)
{
  ...
  if (symTableLookup (entry->label))
  {
    ...
    fprintf (stderr, "ERROR: Label (%s) is not unique\n", entry->label);
    exit (ERROR);
  }
